...
**Current version:** **1.4.3** (see `GlocCameraPlugin.PluginVersion` and [CHANGELOG.md](CHANGELOG.md)).
...
| Shake | `OverspeedEnabled` / `OverspeedStartRatio` / `OverspeedFullRatio` | Overspeed severity uses `speed/maxSpeed` (maxSpeed from aircraft definition, converts km/h→m/s). |
...
5. **Orbit / external:** Leaving cockpit should **restore** post values and **decay** FOV/dolly; shake state resets.
