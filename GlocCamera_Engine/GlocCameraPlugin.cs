...
namespace GlocCamera_Engine
{
    ...
    public sealed class GlocCameraPlugin : BaseUnityPlugin
    {
        ...
        internal static ConfigEntry<float> ShakeSpeedReadingScale { get; private set; }
        internal static ConfigEntry<bool> ShakeOverspeedUseApproximatedIas { get; private set; }
        internal static ConfigEntry<float> ShakeOverspeedIasReferenceDensity { get; private set; }
        ...
        internal static ConfigEntry<bool> LightingFillEnabled { get; private set; }
        internal static ConfigEntry<Vector3> LightingFillLocalPosition { get; private set; }
        internal static ConfigEntry<float> LightingFillPitchDegrees { get; private set; }
        internal static ConfigEntry<float> LightingFillYawDegrees { get; private set; }
        internal static ConfigEntry<float> LightingFillRollDegrees { get; private set; }
        internal static ConfigEntry<float> LightingFillIntensity { get; private set; }
        internal static ConfigEntry<float> LightingFillRange { get; private set; }
        internal static ConfigEntry<float> LightingFillSpotAngle { get; private set; }
        internal static ConfigEntry<float> LightingFillInnerSpotAngle { get; private set; }
        internal static ConfigEntry<float> LightingFillSpotAngleWide { get; private set; }
        internal static ConfigEntry<float> LightingFillInnerSpotAngleWide { get; private set; }
        internal static ConfigEntry<string> LightingFillColorHex { get; private set; }
        internal static ConfigEntry<KeyboardShortcut> LightingFillToggleHotkey { get; private set; }
        internal static ConfigEntry<KeyboardShortcut> LightingFillWideModeHotkey { get; private set; }
        internal static ConfigEntry<float> LightingExternalIntensityMul { get; private set; }
        internal static ConfigEntry<float> LightingExternalRangeMul { get; private set; }
        internal static ConfigEntry<string> LightingExternalNameSubstrings { get; private set; }

        internal static ConfigEntry<float> CockpitViewOffsetLocalX { get; private set; }
        internal static ConfigEntry<float> CockpitViewOffsetLocalY { get; private set; }
        ...
        private void Awake()
        {
            ...
            ShakeOverspeedStartRatio = Config.Bind("Shake", "OverspeedStartRatio", 1.06f,
                "Overspeed severity starts when speed/maxSpeed >= this ratio.");
            ShakeOverspeedFullRatio = Config.Bind("Shake", "OverspeedFullRatio", 1.20f,
            ...
            ShakeSpeedReadingScale = Config.Bind("Shake", "SpeedReadingScale", 1f,
                "Single multiplier on **indicated** speed (m/s and km/h) used for gear/airbrake/touchdown/runway/ground overspeed ratio numerator. 1 = vanilla game speed; <1 reacts earlier, >1 later.");
            ShakeOverspeedUseApproximatedIas = Config.Bind("Shake", "OverspeedUseApproximatedIas", false,
                "If true, overspeed ratio numerator uses game speed × sqrt(ρ/ρ₀) (ρ from aircraft air density, ρ₀ = OverspeedIasReferenceDensity). Crude dynamic-pressure hint, not full instrument IAS.");
            ShakeOverspeedIasReferenceDensity = Config.Bind("Shake", "OverspeedIasReferenceDensity", 1.225f,
                "Reference air density (kg/m³) for sqrt(ρ/ρ₀) when OverspeedUseApproximatedIas is on (sea-level ISA ≈ 1.225).");
            ...
            LightingFillNightMin01 = Config.Bind("Lighting", "FillNightMin01", 0.5f,
                "Instrument flashlight is active when Gloc night blend (same sun heuristic as Atmosphere) is at or above this (0–1).");
            LightingFillEnabled = Config.Bind("Lighting", "FillEnabled", true,
                "Fill: **spawn** on cockpit camera (Fill* in **camera space**), then **immediately** reparent to cockpit with **world pose frozen** — no per-frame pose writes (avoids camera/view glitches).");
            LightingFillLocalPosition = Config.Bind("Lighting", "FillLocalPosition", new Vector3(0f, -0.2f, -0.07f),
                "**Camera-local** position used **only at spawn** before freeze under cockpit.");
            LightingFillPitchDegrees = Config.Bind("Lighting", "FillPitchDegrees", 10f,
                "**Camera-local** euler **X** at spawn only (standard **+10°** toward the panel).");
            LightingFillYawDegrees = Config.Bind("Lighting", "FillYawDegrees", 0f,
                "**Camera-local** euler **Y** at spawn only.");
            LightingFillRollDegrees = Config.Bind("Lighting", "FillRollDegrees", 0f,
                "**Camera-local** euler **Z** at spawn only.");
            LightingFillIntensity = Config.Bind("Lighting", "FillIntensity", 0.52f,
                "Spot intensity (lower = less glare on emissive MFD glass).");
            LightingFillRange = Config.Bind("Lighting", "FillRange", 0.98f,
                "Spot range (meters): keep small so the cone stays on the panel.");
            LightingFillSpotAngle = Config.Bind("Lighting", "FillSpotAngle", 40f,
                "Outer cone (degrees) — **normal** (narrow) flashlight mode.");
            LightingFillInnerSpotAngle = Config.Bind("Lighting", "FillInnerSpotAngle", 24f,
                "Inner cone (degrees) in **normal** mode; must stay below FillSpotAngle.");
            LightingFillSpotAngleWide = Config.Bind("Lighting", "FillSpotAngleWide", 72f,
                "Outer cone (degrees) in **wide** flashlight mode (after hotkey toggle).");
            LightingFillInnerSpotAngleWide = Config.Bind("Lighting", "FillInnerSpotAngleWide", 46f,
                "Inner cone (degrees) in **wide** mode; must stay below FillSpotAngleWide.");
            LightingFillColorHex = Config.Bind("Lighting", "FillColorHtml", "#FFE8CC",
                "Spot color as HTML (#RRGGBB or named color).");
            LightingFillToggleHotkey = Config.Bind("Lighting", "FillToggleHotkey", new KeyboardShortcut(KeyCode.J),
                "Cockpit + controls on: toggle fill **armed** (night gate still applies). Rebind in cfg.");
            LightingFillWideModeHotkey = Config.Bind("Lighting", "FillWideModeHotkey", new KeyboardShortcut(KeyCode.K),
                "Cockpit + controls on: toggle **wide** vs **normal** cone. Rebind in cfg.");
            LightingExternalIntensityMul = Config.Bind("Lighting", "ExternalIntensityMul", 1.35f,
                "Multiply intensity on matched external lights (see ExternalNameSubstrings).");
            ...
            CockpitViewOffsetLocalY = Config.Bind("CockpitView", "OffsetLocalY", 0f,
                "Additive local camera Y.");
            CockpitViewOffsetLocalZ = Config.Bind("CockpitView", "OffsetLocalZ", 0f,
                "Additive local camera Z (applied together with dolly: final Z = G-LOC dolly + OffsetLocalZ when TrackIR is off).");
            CockpitViewFovBiasDegrees = Config.Bind("CockpitView", "FovBiasDegrees", 0f,
                "Additive degrees on cockpit FOV after vanilla clamp + G-LOC longitudinal offset; clamped again to cockpit min/max.");
