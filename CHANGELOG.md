...
### Changed
- **ApexView restored** to classic **ControlInputs** local X/Y/Z offset (same as GitHub **2.0.9**). Removed DCS/neck-inertia experiments (2.1.x–2.2.0). Delete `[ApexView]` in cfg to get default scales back.

## 2.2.0 — 2026-05-30

### Changed
- **ApexView rewritten** (custom neck inertia, DCS reference video — not View.lua port): bank from `SignedAngle` (no euler flip at inverted), **pan/tilt add only**, local roll **always 0** (fixes 180° spins).
- Into-turn pan = `-IntoTurnGain × bank`; look-up tilt ramps with bank + optional pitch/G assist.
...
### Fixed
- **ApexView pitch inverted:** DCS positive vertical angle now maps to **look up** in NO (`VerticalAngleSign` default **-1**), fixing “stare at floor/instruments” in hard turns.
...
- **head_roll_shaking:** counter-roll sign aligned with DCS (+folded×gain).
- **omz:** clamped (`OmzMaxRadPerSec` default **0.35**) before the ×45 term to limit vertical spikes.

## 2.1.0 — 2026-05-30

### Changed
- **ApexView v2 (DCS View.lua):** replaced stick-driven local offset with **NaturalHeadMoving** head inertia — horizontal from bank (`-0.25×roll` with >90° fold), vertical from pitch + body yaw rate (`0.4×tang + 45×omz`, clamp 90°), applied as **local rotation** after vanilla pan/tilt.
- **head_roll_shaking:** optional counter-roll compensation (gain **0.3**, max **30°**).
- **CockpitNyMove:** optional local Y shift from normal G (`|Dot(accel, up)|/9.81`).
- **CockpitOffsetsMin/Max** clamp on final cockpit local position (DCS defaults ±0.2 / ±0.25 / ±0.5 m).
